...
def test_find_combinations():
    ...
    Source of reference values: calculatorsoup.com.
    """
...
def test_create_pentagon_numbers():
    ...
    assert lpe.create_pentagon_numbers(100) == {0, 1, 35, 5, 70,
                                                12, 51, 22, 92}
...
def test_get_permutations():
    ...
    assert lpe.get_permutations('abc') == ['cba', 'bca', 'bac',
                                           'cab', 'acb', 'abc']
    assert lpe.get_permutations('') == ['']
    assert lpe.get_permutations('abcd') == ['dcba', 'cdba', 'cbda', 'cbad',
                                            'dbca', 'bdca', 'bcda', 'bcad',
                                            'dbac', 'bdac', 'badc', 'bacd',
                                            'dcab', 'cdab', 'cadb', 'cabd',
                                            'dacb', 'adcb', 'acdb', 'acbd',
...
def test_equal_division():
    ...
    assert lpe.equal_division(sample, 1) == ['a', 'b', 'c', 'd', 'e', 'f',
                                             'g', 'h', 'i', 'j', 'k', 'l',
                                             'm', 'n', 'o', 'p', 'q', 'r',
                                             's', 't', 'u', 'v', 'w', 'x',
                                             'y', 'z']
    assert lpe.equal_division(sample, 2) == ['ab', 'cd', 'ef', 'gh', 'ij',
                                             'kl', 'mn', 'op', 'qr', 'st',
                                             'uv', 'wx', 'yz']
...
def is_lychrel_number(num, cache=None):
    """Return True if num is a Lychrel number.
    Cache is num = k, v = True if Lychrel.
    """
    if cache is None:
        cache = {}

    def add_falses_to_cache(pt_thread):
        """Add non-Lychrel numbers to cache."""
        new_falses = {}
        for num in pt_thread:
            new_falses[num] = False
        return new_falses

    def add_trues_to_cache(l_thread):
        """Add Lychrel numbers to cache."""
        new_trues = {}
        for num in l_thread:
            new_trues[num] = True
        return new_trues
    ...
    temp = num
    thread = [num]
    limit = iteration_limit_a.get_value()
    while thread_length < limit:
        temp = reverse_and_add(temp)
        thread.append(temp)
        if is_palindrome(temp):
            new_limit = len(thread) * int(2.5)
            if limit < new_limit:
                iteration_limit_a.set_value(new_limit)
            cache = cache.update(add_falses_to_cache(thread))
            return False

        thread_length += 1
    else:
        cache = cache.update(add_trues_to_cache(thread))
        return True
